#!/bin/bash
declare -rg sourcerRoot="./sourcer.source_test_sh/" 
declare -rg sourcerFile="$sourcerRoot/file/"
if ! [[ -e "$sourcerRoot/base/assert.source.sh" ]]; then 
	"$sourcerRoot/config.sh" >/dev/null
fi

source "$sourcerRoot/base/sourcer.source.sh"
source "$sourcerRoot/base/assert.source.sh"

test_sourcer__visit_filepath_echo(){
	# remember order matters!
	assert_output_true test_sourcer__visit_filepath_echo_pkgA_pkgB_output \
		---  sourcer__visit_filepath_echo "$sourcerFile"/compose_pkgA_pkgB/base 
}


test_sourcer_compose_pkgA_pkgB_success(){
	assert_output_true test_sourcer__visit_filepath_echo_pkgA_pkgB_output \
		--- assert_true 'sourcer_compose "$sourcerFile/compose_pkgA_pkgB"'
}


test_sourcer__visit_filepath_echo_pkgA_pkgB_output(){
	echo "$sourcerFile/compose_pkgA_pkgB/base/pkgA.source.sh"
	echo "$sourcerFile/compose_pkgA_pkgB/base/pkgB.source.sh"
}


test_sourcer_compose_level_max(){
	assert_true 'sourcer_compose "$sourcerFile"/compose_level_max'
}


test_sourcer_compose_level_max_exceeded(){
	assert_output_true assert_false 'sourcer_compose "$sourcerFile"/compose_level_max_exceeded' \
...
Error: Ignoring components located in subdirectory: '$sourcerFile/compose_level_max_exceeded/base/subcomponent_2//base/subcomponent_3//base/subcomponent_4//base/subcomponent_5//base/subcomponent_6/' because
...
test_sourcer_compose_component_max_exceeded()(
	...
	assert_output_true sourcer_compose "$sourcerFile"/component_max_exceeded \
...
Error: Maximum component count: 3 exceeded for a given level
  +    while starting to process directory: '$sourcerFile/component_max_exceeded/base/component_4/'.  This and all
...
test_sourcer_compose_immediate_branch_error(){
	assert_output_true sourcer_compose "$sourcerFile"/compose_immediate_branch_error \
...
Error: Ignoring components located in subdirectory: 'base' of: '$sourcerFile/compose_immediate_branch_error/base/base/'.
